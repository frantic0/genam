...
def simulated_annealing(objective, bounds, n_iterations, step_size, temperature, best = None):
    '''
        ...
        If the new points is worse than the current candidate, it can still be accepted.

        The likelihood of accepting a solution worse than the current solution is a function of the temperature of the search
        and how much worse the solution is than the current solution.

    '''
    if best == None:                                                                # if initial solution is not provided
        best = bounds[:, 0] + rand( len(bounds) ) * ( bounds[:, 1] - bounds[:, 0] ) # generate a random initial point (uniform distribution)
    ...
    for i in range(n_iterations):
        ...
        if candidate_eval < best_eval:                                              # check for new best solution
            ...
            diff = candidate_eval - current_eval                                    # difference between candidate and current point evaluation
            metropolis = exp( -diff / (temperature / float(i + 1)) )                # calculate likelihood of accepting a solution with worse performance (metropolis acceptance criterion)
